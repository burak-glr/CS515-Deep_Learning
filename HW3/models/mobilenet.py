...
class MobileNetV2(nn.Module):
    ...
    Strides in stage 2 and the initial conv are changed from 2→1 to keep
    ...
    def _make_layers(self, in_planes: int) -> nn.Sequential:
        """Build all inverted residual stages."""
        layers = []
    ...
    def get_features(self, x: torch.Tensor) -> torch.Tensor:
        """Extract the 1280-d pooled feature vector before the linear head.

        Args:
            x: Input tensor (N, 3, 32, 32).

        Returns:
            Feature tensor (N, 1280).
        """
        out = F.relu(self.bn1(self.conv1(x)))
        out = self.layers(out)
        out = F.relu(self.bn2(self.conv2(out)))
